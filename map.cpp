...
//搜索路径
bool FindPath(Graph &map, int start, int end);
...
bool FindPath(Graph &map, int start, int end, vector<int> &arr) {
  ...
  while (cur != nullptr) {
    if (cur->vertex == end) {
      ...
      if (check) {
        findIt = FindPath(map, cur->vertex, end, arr);
      }
    }
    cur = cur->next;
    check = true;
...
void SearchPath(Graph &map) {
  ...
  int start = min(a, b);
  int end = max(a, b);
  bool check = FindPath(map, start, end, arr);
  if (check) {
    cout << "Done" << endl;
  } else {
    cout << "Not find" << endl;
