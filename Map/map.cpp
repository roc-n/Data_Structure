...
//  递归查找路径
void FindPath(Graph &map, int start, int end, bool findIt);
//  搜索路径
...
void FindPath(Graph &map, int start, int end, vector<int> &arr, bool &findIt) {
...
void SearchPath(Graph &map) {
  ...
  int start = min(a, b);
  int end = max(a, b);
  FindPath(map, start, end, arr, finfIt);
  if (finfIt) {
    cout << "Done" << endl;
  } else {
    cout << "Not find" << endl;
